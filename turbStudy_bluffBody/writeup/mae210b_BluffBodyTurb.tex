...
In spite of its deficiencies, however, DES proves to be an extremely effective method for bluff body simulations today, especially when compared to higher-order methods that alleviate that issues mentioned above.
...
computational efficiency is king. RANS ok for time-averaged loads in some situations. DES is better than LES for simulations of the same computational resources.

DES is not the final solution. Grid ambiguity, order of accuracy. SAS can be a remedy but isn't a solution either.

Future is computationally feasible LES. DNS will remain a laboratory for a long time.
